...
% nt --- --- severity of change
    ...
    methods
        %% Initialization
        function obj = DF11(ft,nt,gen,Dec,preEvolution,AddProper)
            if nargin > 0
                N = size(Dec,1);
                obj(1,N) = DF11;
              ...
              G = abs(sin(0.5*pi*t));
              y1 = pi/6 *G + (pi/2-pi/3*G)*Dec(:,1);
              y2 = pi/6 *G + (pi/2-pi/3*G)*Dec(:,2);
              g = 1 + G + sum((Dec(:,3:end)- 0.5*G*Dec(:,1)).^2,2); 
                Obj(:,1) = g.* (sin(y1)) ;
                Obj(:,2) = g.* (sin(y2).*cos(y1)) ;
                Obj(:,3) = g.* (cos(y2).*cos(y1));
       ...
       %% Sample reference points on Pareto front
        function P = PF(ft,nt,maxgen,preEvolution)
            ...
            for i = 1 : ceil((maxgen-preEvolution)/ft+1)
                ...
                G = abs(sin(0.5*pi*t));
                g = 1 + G;
                y1 = g*sin(pi/6 *G + (pi/2-pi/3*G)*X(:,1));
                MaxY = max(y1);
                MinY = min(y1);
                pf = X.*(g);
                pf(pf(:,1)>MaxY,:) = [];
