...
% nt --- --- severity of change
    ...
    methods
        %% Initialization
        function obj = DF8(ft,nt,gen,Dec,preEvolution,AddProper)
            if nargin > 0
                N = size(Dec,1);
                obj(1,N) = DF8;
              ...
              alfa = 2.25 + 2 * cos(0.5*pi*t);
              G = sin(0.5*pi*t);
              belta = 100*(G.^2);
              g = 1 + sum((Dec(:,2:end)- (G*sin(4*pi*Dec(:,1).^belta))./(1+abs(G))).^2,2);

                Obj(:,1) = g.* (Dec(:,1) + 0.1*sin(3*pi*Dec(:,1)));
       ...
       %% Sample reference points on Pareto front
        function P = PF(ft,nt,maxgen,preEvolution)
            ...
            for i = 1 : ceil((maxgen-preEvolution)/ft+1)
                t  = (i-1) / nt;
                alfa = 2.25 + 2 * cos(0.5*pi*t);
                P1 = Dec + 0.1 * sin(3*pi*Dec);
                P2 = (1 - Dec + 0.1 * sin(3*pi*Dec)).^alfa;
                pf(:,1) = P1 ;
                pf(:,2) = P2 ;
                P(i) = struct('PF',pf);
            end 
