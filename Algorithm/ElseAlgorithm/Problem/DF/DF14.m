...
% nt --- --- severity of change
    ...
    methods
        %% Initialization
        function obj = DF14(ft,nt,gen,Dec,preEvolution,AddProper)
            if nargin > 0
                N = size(Dec,1);
                obj(1,N) = DF14;
              ...
              Obj(:,1) = g.* (1 - y1 + 0.05*sin(6*pi*y1));
              Obj(:,2) = g.* (1 - Dec(:,2) + 0.05*sin(6*pi*Dec(:,2))).*(y1 + 0.05*sin(6*pi*y1)) ;
              Obj(:,3) = g.* (Dec(:,2) + 0.05*sin(6*pi*Dec(:,2))).*(y1 + 0.05*sin(6*pi*y1));
       ...
       %% Sample reference points on Pareto front
        function P = PF(ft,nt,maxgen,preEvolution)
            ...
            for i = 1 : ceil((maxgen-preEvolution)/ft+1)
              ...
              G = sin(0.5*pi*t);
              y1 = 0.5 + G*(X(:,1)-0.5);
              pf(:,1) = (1 - y1 + 0.05*sin(6*pi*y1));
              pf(:,2) = (1 - X(:,2) + 0.05*sin(6*pi*X(:,2))).*(y1 + 0.05*sin(6*pi*y1));
              pf(:,3) = (X(:,2) + 0.05*sin(6*pi*X(:,2))).*(y1 + 0.05*sin(6*pi*y1)) ;
              pf(NDSort(pf,1)~=1,:) = [];    
                pf = pf ;
                P(i) = struct('PF',pf);
            end 
