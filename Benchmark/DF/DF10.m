...
% nt --- --- severity of change
    ...
    methods
        %% Initialization
        function obj = DF10(ft,nt,gen,Dec,preEvolution,AddProper)
            if nargin > 0
                N = size(Dec,1);
                obj(1,N) = DF10;
              ...
              H = 1.25 + 1 * cos(0.5*pi*t);
              G = sin(0.5*pi*t);
              g = 1 + sum((Dec(:,3:end)- G).^2,2);

                Obj(:,1) = g.* (sin(0.5*pi*Dec(:,1))).^H ;
                Obj(:,2) = g.* (sin(0.5*pi*Dec(:,2)).*cos(0.5*pi*Dec(:,1))).^H;
                Obj(:,3) = g.* (cos(0.5*pi*Dec(:,2)).*cos(0.5*pi*Dec(:,1))).^H;
