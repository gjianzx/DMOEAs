...
% nt --- --- severity of change
    ...
    methods
        %% Initialization
        function obj = FDA4(ft,nt,gen,Dec,preEvolution,AddProper)
            if nargin > 0
                N = size(Dec,1);
                obj(1,N) = FDA4;
              ...
              if gen < preEvolution
                ...
                M = 3;
                G = abs(sin(0.5*pi*t));
                g = sum((Dec(:,M:end)-G).^2,2);
                Obj = repmat(1+g,1,M).*fliplr(cumprod([ones(size(g,1),1),cos(Dec(:,1:M-1)*pi/2)],2)).*[ones(size(g,1),1),sin(Dec(:,M-1:-1:1)*pi/2)];
    ...
    methods (Static)
        ...
        function P = PF(ft,nt,maxgen)
            pf = UniformPoint(1500,3);
            pf = pf./repmat(sqrt(sum(pf.^2,2)),1,3);
            P(1) = struct('PF',pf);
        end
    end
end